...
Server::Server()
    ...
    commonQueriesMap["createGame"] = [&](const json &body, User &user) -> json {
        ...
        Server::Server()
            ...
            commonQueriesMap["createGame"] = [&](const json &body,
                                                 User &user) -> json {
                // basic checks
                if (!body.contains("dictionaryName") ||
                    !body["dictionaryName"].is_string()) {
                    return {
                        {"header", {{"type", "wrongFormatError"}}},
                        {"body", {{"text", "can't find \"dictionaryName\""}}}};
                }

                if (!body.contains("parserName") ||
                    !body["parserName"].is_string()) {
                    return {{"header", {{"type", "wrongFormatError"}}},
                ...
                if (body.contains("autoJoin") &&
                    body["autoJoin"].is_boolean() && body["autoJoin"]) {
            ...
            commonQueriesMap["joinGame"] = [&](const json &body,
                                               User &user) -> json {
                ...
                if (game->hasUser(user.getId())) {
                    return {
                        {"header", {{"type", "connectAfterLeaveError"}}},
            ...
            commonQueriesMap["leaveGame"] = [&](const json &body,
                                                User &user) -> json {
                if (user.getGame() == nullptr) {
                ...
                return {{"header", {{"type", "GameLeaveSuccessfully"}}},
                        {"body", {}}};
            };
            commonQueriesMap["getNewLine"] = [&](const json &body,
                                                 User &user) -> json {
                if (user.getGame() == nullptr) {
                    return {{"header", {{"type", "notInGameError"}}},
                            {"body", {{"text", "not in game"}}}};
            ...
            commonQueriesMap["addNewChar"] = [&](const json &body,
                                                 User &user) -> json {
                if (user.getGame() == nullptr) {
                    return {{"header", {{"type", "notInGameError"}}},
                            {"body", {{"text", "not in game"}}}};
            ...
            commonQueriesMap["backspace"] = [&](const json &body,
                                                User &user) -> json {
                if (user.getGame() == nullptr) {
                    return {{"header", {{"type", "notInGameError"}}},
                            {"body", {{"text", "not in game"}}}};
            ...
            commonQueriesMap["getStates"] = [&](const json &body,
                                                User &user) -> json {
                if (user.getGame() == nullptr) {
                    return {{"header", {{"type", "notInGameError"}}},
                            {"body", {{"text", "not in game"}}}};
            ...
            commonQueriesMap["exit"] = [&](const json &body,
                                           User &user) -> json {
                user.setWillToExit();
                return {};
            };
        ...
        void Server::parseQuery(tcp::socket s) {
            try {
                ...
                try {
                    ...
                    if (query["header"]["type"] != "hello") {
                        json result = {
                            ...
                            {"body",
                             {{"text", "first query should be hello"}}}};
                        client << result << '\n';
                        return;
                    }
                    ...
                    std::cerr << e.what()
                              << std::endl;  // process error to client
                    return;
                }
                ...
                User user(
                    user_name,
                    userStorage
                        .get());  // after you parse login and register query
                ...
                try {
                    while (client) {
                        ...
                        if (auto it =
                                commonQueriesMap.find(queryHeader["type"]);
