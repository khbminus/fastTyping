...
struct AbstractParser {
    virtual bool isCorrect(const std::string &inputWord,
                           const std::string &dictionaryWord) const;
};

struct AbstractDictionaryLoader {
    virtual std::unique_ptr<AbstractDictionary> loadDictionary(
...
struct ConstDictionary : AbstractDictionary {
    ...
    explicit ConstDictionary(std::vector<QString> words_)
